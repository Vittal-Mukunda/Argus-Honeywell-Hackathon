...
// ARGUS C2: stash this frame's SuperPoint detections (pre-sorted by score desc).
void FeatureTracker::setExternalKeypoints(const vector<cv::Point2f> &pts)
{
    external_kpts = pts;
    has_external_kpts = true;
}

// ARGUS C2: pick up to n_max SuperPoint keypoints to add as NEW features, in
// place of goodFeaturesToTrack. setMask() has already painted MIN_DIST exclusion
// discs (value 0) around the features still tracked by LK, so we (a) keep only
// detections landing on free mask pixels (==255) and (b) enforce MIN_DIST spacing
// between the picks themselves -- exactly goodFeaturesToTrack's masked + spaced
// behaviour, but seeded by the learned detector. external_kpts is score-sorted,
// so the greedy walk prefers high-confidence keypoints.
vector<cv::Point2f> FeatureTracker::selectExternalKeypoints(int n_max)
{
    vector<cv::Point2f> picked;
    if (n_max <= 0 || external_kpts.empty() || mask.empty())
        return picked;
    cv::Mat m = mask.clone();   // local copy: don't disturb the shared mask
    for (const auto &p : external_kpts)
    {
        if (static_cast<int>(picked.size()) >= n_max)
            break;
        ...
        {
            picked.push_back(p);
            cv::circle(m, cv::Point(x, y), MIN_DIST, 0, -1);
        }
    }
    return picked;
}
    ...
    if (1)
    {
        ...
        if(!USE_GPU)
        {
            if (n_max_cnt > 0)
            {
                ...
                // ARGUS C2: when SuperPoint is driving the front-end and this
                // frame's detections arrived, seed new features from the learned
                // detector; otherwise fall back to Harris goodFeaturesToTrack.
                if (USE_SUPERPOINT && has_external_kpts)
                    n_pts = selectExternalKeypoints(n_max_cnt);
                else
        ...
        TicToc t_a;
        addPoints();
        has_external_kpts = false;   // ARGUS C2: consume per-frame; no stale reuse
