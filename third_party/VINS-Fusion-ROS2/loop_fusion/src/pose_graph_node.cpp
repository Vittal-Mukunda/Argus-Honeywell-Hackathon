...
int main(int argc, char **argv)
{
    ...
    // ARGUS patch: was `argc != 2`. ros2 launch appends `--ros-args -r ...` so
    // argc > 2 (the config is still argv[1]); the strict `!= 2` took the early
    // `return 0`, which then ran ~PoseGraph() -> t_optimization.detach() on a
    // never-started thread -> std::system_error "Invalid argument" -> SIGABRT.
    if(argc < 2)
    {
