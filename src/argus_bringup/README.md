...
| `config/argus_bridge.yaml` | The frozen 12-entry gz↔ROS bridge: stereo `cam{0,1}` image+info, `imu` (250 Hz), `lidar/points`+`lidar/scan` (10 Hz), `rangefinder` (20 Hz), `ground_truth/pose` (100 Hz), `cmd_vel`, and dual `/clock`+`/argus/clock`. |
